...
<head>
    ...
    <link rel="shortcut icon" href="{{URL('/')}}/assets/images/eis.png">
    <!-- Bootstrap Css -->
...
<script>
    ...
    function checkDatabase() {
        $.ajax({
            url: "{{ route('notification.todayPeningBookings') }}",
            method: 'GET',
            success: function(response) {
                if (response.status === 'not empty') {
                    var audio = new Audio('{{asset("assets/sound/sound.mp3")}}'); // Replace with the path to your sound file
                    audio.play();

                    $('#bell').text(response.total);
                              // toast
                    ...
                    var url = `{{ url('BookingEdit') }}/${item.id}`;
                    var htmlContent=`
                            ...
                            <div class="media">              
                                <div class="media-body">                   
                                    ...
                                    <div class="font-size-12 text-muted">                          
                                        <p class="mb-1" key="t-grammer">${item.PartyName}</p>                        
                                    </div>                    
                                </div>                    
                            </div>             
                        </a>`;
                        ...
                        $('#grid').append(htmlContent);




                        // Perform any other operations you need with item
                    });          
         ...
         // Command: toastr["success"]("New lead is there. Please check and assign")
            ...
            error: function() {
                console.error('Error checking the database.');
            }
        });
        ...
        $.ajax({
            url: "{{ route('notification.peningBookings') }}",
            method: 'GET',
            success: function(response) {
                if (response.status === 'not empty') {
                    var audio = new Audio('{{asset("assets/sound/sound.mp3")}}'); // Replace with the path to your sound file
                    ...
                    $('#booking_payment').text(response.total);
                              // toast
                    ...
                    var htmlContent=`
                        ...
                        $('#grid_booking').append(htmlContent);
    ...
    setInterval(checkDatabase, 300000); // 1 min interval 
