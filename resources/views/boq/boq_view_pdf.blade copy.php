...
<head>
    ...
    <style type="text/css">
        ...
        .table-li{
            margin-top:4px;line-height:0.1%; margin-bottom: 4px; margin-left: 0px;list-style-type: none; list-style-position: inside;padding-left: 20px; padding-top: 10px;
        }
    </style>
...
<body>
        ...
        <tr>
            <td valign="bottom" width="70%">
                ...
                <span> {{ $estimate[0]->PartyName }}
                    {{ $estimate[0]->WalkinCustomerName == 1 ? ' -' . $estimate[0]->WalkinCustomerName : '' }}</span><br>
                {{ $estimate[0]->TRN != null ? 'TRN: ' . $estimate[0]->TRN : '' }}<br>
    ...
    @foreach ($services as $service)
        ...
        <div
            style="background-color: #c9daf8;text-align:center; padding:10px; border-top:1px solid;border-left:1px solid;border-right:1px solid; margin-top:20px">
        ...
        <table width="100%" style=" border: 1px solid black; border-collapse: collapse;">
            <thead>
                <tr style="background-color: #f1f1f1; border-bottom: 1px solid;">
                    ...
                    <th width="10%" class="quantity table-th">Unit <br> Cost</th>
                    <th width="10%" class="quantity table-th">VAT <br> per Unit</th>
                    <th width="10%" class="quantity table-th">Unit Cost <br> <small>(incl. VAT)</small></th>
                    <th width="10%" class="quantity table-th">Unit <br> Margin</th>
                    <th width="10%" class="quantity table-th">Selling <br> Price</th>
                    <th width="10%" class="quantity table-th">Quantity</th>
                    <th width="10%" class="quantity table-th">Total <br> Cost</th>
                    <th width="10%" class="quantity table-th">Total <br> Margin</th>
                    <th width="15%" class="price table-th">Total <br> Price</th>
                </tr>
            </thead>
            ...
            <tbody>
                ...
                @foreach ($estimate_detail as $key => $value)
                    ...
                    <tr valign="top">
                        <td height="13px" style="text-align:center;border-right: 1px solid black; border-left: 1px solid black;">
                            {{ $i++ }}
                        </td>
                        <td style="border-right: 1px solid black; border-left: 1px solid black; font-size:11px">
                            @if ($value->ItemName == 'Heading')
                                ...
                                @endforeach
                            @else
                                <li class="table-li">{{ $value->Description }}</li>
                            @endif

                        </td>

                        <td class="table-td"> 
                            <li class="table-li">{{  number_format($value->unit_net_cost, 2)  }}</li>
                        </td>
                        <td class="table-td"> 
                            <li class="table-li">{{  number_format($value->vat_per_unit, 2) }}</li>
                        </td>
                        <td class="table-td"> 
                            <li class="table-li">{{ number_format($value->unit_with_vat, 2) }}</li>
                        </td>
                        <td class="table-td"> 
                            <li class="table-li">{{ number_format($value->per_unit_profit_margin, 2) }}</li>
                        </td>
                        <td class="table-td"> 
                            <li class="table-li">{{ number_format($value->per_unit_selling_price, 2) }}</li>
                        </td>
                        <td class="table-td"> 
                            <li class="table-li">@if ($value->LS == 'NO') {{ $value->Qty }} @else  L/S @endif</li>
                        </td>
                        <td class="table-td">
                            <li class="table-li" > {{ number_format($value->total_cost_with_vat, 2) }}</li>
                        </td>
                        <td class="table-td">
                            <li class="table-li" > {{ number_format($value->total_profit_margin, 2) }}</li>
                        </td>
                        <td class="table-td">
