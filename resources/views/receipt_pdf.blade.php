        ...
        <table width="100%" style="border-collapse: collapse; margin-top: 20px;" >
            <tr>
                ...
                <td style="width: 70%; text-align: left; padding-top: 10px;">
                    <img src="{{ asset('assets/images/header1.jpg') }}" width="150%">
                </td>
                ...
                <td style="width: 30%; text-align: right; padding-top: 20px; font-size: 18px; line-height: 1;">
                    <table style="width: 100%;">
                        <tr >
                            <th style="text-align: right;font-size:24px;color: #163263;padding-bottom:10px" colspan="2">Receipt</th>
                        </tr>
                        {{-- <tr>
                            <th style="text-align: left; font-size:14px;color: #325da7;">Estimate No:</th>
                            <td style="text-align: right; font-size:14px;color: #325da7;">{{ $invoice_mst[0]->EstimateNo }}</td>
                        </tr> --}}
                        <tr>
                            <th style="text-align: left;font-size:14px;color: #325da7;">Reference No:</th>
                            <td style="text-align: right;font-size:14px;color: #325da7;">{{ $invoice_mst[0]->InvoiceNo }}</td>
                        </tr>
                        {{-- <tr>
                            <th style="text-align: left;font-size:14px;color: #325da7;">Creation Date:</th>
                            <td style="text-align: right;font-size:14px;color: #325da7;">{{ date('d-M-Y', strtotime($invoice_mst[0]->Date)) }}</td>
                        </tr> --}}
                        <tr>
                            <th style="text-align: left;font-size:14px;color: #325da7;">Date:</th>
                            <td style="text-align: right;font-size:14px;color: #325da7;">{{ date('d-M-Y') }}</td>
                        </tr>
                        {{-- @if($invoice_mst[0]->ExpiryDate)
                        <tr>
                            <th style="text-align: left;font-size:14px;color: #325da7;">Expiry Date:</th>
                            <td style="text-align: right;font-size:14px;color: #325da7;">{{ date('d-M-Y', strtotime($invoice_mst[0]->ExpiryDate)) }}</td>
                        </tr>
                        @endif --}}
...
<table  width="732px" style="margin-top:30px;" >
    <tr>
        ...
        <td width="45%" style="border: 1px solid black; ">
            <table width="100%" style="border-collapse: collapse;">
                ...
                <tr>
                    <th style="text-align: left; font-size: 14px; padding: 10px 10px 0px 10px;width: 20%;">Address:</th>
                    <td style="text-align: left; font-size: 14px; padding: 10px 10px 0px 10px;width: 80%;">{{ $party->Address}}</td>
                </tr>
                @endif
                @if($party->Mobile)
                <tr>
    ...
    @foreach ($services as $service)
        ...
        <div width="745px" style="background-color: #c9daf8;text-align:center; padding:10px; border-top:1px solid;border-left:1px solid;border-right:1px solid; margin-top:20px">
            <b>{{ strtoupper($service->name) }}</b>
        </div>

        <table width="733px" style=" border: 1px solid black; border-collapse: collapse;">
            <thead>
                <tr style="background-color: #f1f1f1; border-bottom: 1px solid;">
                    ...
                    <th class="product"
                        style="border-right: 1px solid black; border-left: 1px solid black; text-align:left; padding-left:10px">
                        Description
                    </th>
            ...
            <tbody>
                ...
                @foreach ($estimate_detail as $key => $value)
                    ...
                    <tr valign="top">
                        ...
                        <td class="table-td " style="text-align: left; padding-left:5px" > {{ $value->Description }}
    ...
    @endforeach
    <table class="" width="100%" style="margin-top: 20px;">
        <tr >
            ...
            <div style="margin-top:10px;"></div>
                        <tr class="cart_subtotal">
                            ...
                            <td style="border-top:1px solid;padding-top:10px" class="price"><span class="totals-price"><span class="amount">
                                        {{ number_format($invoice_mst[0]->SubTotal, 2) }}</span></span>
                            </td>
                        </tr>
         
               
            @if($invoice_mst[0]->DiscountAmount > 0)
                <tr class="order_total">
                    <td class="no-borders"></td>
                    <th class="description">Discount</th>
                    <td class="price"><span class="totals-price"><span
                                class="amount">{{ number_format($invoice_mst[0]->DiscountAmount, 2) }}</span></span>
                    </td>
                </tr>
            @endif
            <tr class="order_total">
                ...
                <td class="price"><span class="totals-price"><span
                            class="amount">{{ number_format($invoice_mst[0]->Total, 2) }}</span></span>
                </td>
            ...
            <tr class="order_total">
                ...
                <th class="description">Tax <span
                        style="font-size: 10px;">({{ substr($invoice_mst[0]->TaxType, 3, 10) }})</span>
                </th>
                <td class="price"><span class="totals-price"><span
                            class="amount">{{ number_format($invoice_mst[0]->Tax, 2) }}</span></span>
                </td>
            </tr>
            ...
            <tr class="order_total">
                ...
                <td class="price"><span class="totals-price"><span
                            class="amount">{{ number_format($invoice_mst[0]->GrandTotal, 2) }}</span></span>
                </td>
            </tr>
            ...
            <tr class="order_total">
                ...
                <td class="price" style="color:green"><span class="totals-price"><span
                            class="amount">{{ number_format($invoice_mst[0]->Paid, 2) }}</span></span>
                </td>
            </tr>
            <tr class="order_total">
                <td class="no-borders"></td>
                <th class="description">Balance</th>
                <td class="price " style="color:red"><span class="totals-price"><span
                            class="amount "><b>{{ number_format($invoice_mst[0]->Balance, 2) }}</b></span></span>
                </td>
            </tr>
    ...
    <table style="width: 100%; border: 1px solid black; border-collapse: collapse; margin-top: 90px;page-break-inside: avoid">
        <thead>
            <tr>
                <th style="background-color: #c9daf8; text-align:center; padding:10px; border:1px solid #000; margin-top:20px; width:745px;" colspan="5">Payment Histroy</th>
            </tr>
            <tr style="background-color: #f1f1f1;">
                <th style="border: 1px solid black; padding: 8px; width: 5%;">S#</th>
                <th style="border: 1px solid black; padding: 8px; width: 15%;">Date</th>
                <th style="border: 1px solid black; padding: 8px; width: 20%;">Payment Mode</th>
                <th style="border: 1px solid black; padding: 8px; width: 40%;">Description</th>
                <th style="border: 1px solid black; padding: 8px; width: 20%;">Amount Received</th>
            </tr>
        </thead>
        <tbody>
            <?php $i = 1; ?>
            ...
            <tr style="border-bottom: 1px solid #ccc;">
                <td style="padding: 8px; text-align: center;border:1px solid #000;">{{ $i++ }}</td>
                <td style="padding: 8px; text-align: center;border:1px solid #000;">{{ date('d-m-Y', strtotime($receipt->date)) }}</td>
                <td style="padding: 8px; text-align: center;border:1px solid #000;">{{ $receipt->PaymentMode }}</td>
                <td style="padding: 8px;border:1px solid #000;">{{ $receipt->Description }}</td>
                <td style="padding: 8px; text-align: right;border:1px solid #000;">{{ number_format($receipt->Paid, 2) }}</td>
            </tr>
            @endforeach

            {{-- Uncomment this block to show the total row --}}
            <tr style="background-color: #f1f1f1;">
                <td colspan="4" style="text-align: right; padding: 10px; border: 1px solid black;"><b>Total</b></td>
                <td style="text-align: right; padding: 10px; border: 1px solid black;"><b>{{ number_format($invoice_mst[0]->Paid, 2) }}</b></td>
            </tr>
        </tbody>
    </table>
