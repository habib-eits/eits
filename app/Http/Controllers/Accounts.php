    ...
    public function UserVerify(request $request)
    {
        ...
        $company = DB::table('company')->get();
            ...
            if(Session::get('UserType') == 'SuperAdmin'){
                return redirect('super-admin-dashboard')->with('error', 'Welcome to ' . session::get('CompanyName') . ' Software')->with('class', 'success');
            }
            if(Session::get('UserType') == 'Admin'){
                return redirect('admin-dashboard')->with('error', 'Welcome to ' . session::get('CompanyName') . ' Software')->with('class', 'success');
            }
            
            
            else{
                return redirect('agent-dashboard')->with('error', 'Welcome to ' . session::get('CompanyName') . ' Software')->with('class', 'success');
            }    
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<div class="d-flex align-items-center col-actions">
                        <a href="' . URL('/InvoicePDF/' . $row->InvoiceMasterID) .'"><i class="font-size-18 me-1 mdi mdi-file-pdf-outline align-middle me-1 text-secondary"></i></a></div>';
...
// where InvoiceType=".$request->InvoiceTyp." and BranchID=".$request->BranchID."');
    ...
    //     // 'exp_chart'
    //     return view('dashboard', compact('pagetitle', 'v_cashflow', 'invoice_master', 'expense', 'revenue', 'profit_loss', 'cash', 'cash1', 'sale','total_booking','total_leads','leads_won','leads_lost','leads_pending','leads_new','leads_reject','booking_payment'));
    // }
  ...
  public function ajax_party_save(request $request)
  {
    ...
    public  function PartiesUpdate1(request $request)
    {
        ...
        $id = DB::table('party')->where('PartyID', $request->input('PartyID'))->update([
            'PartyName' => $request->PartyName,
            'Phone' => $request->Phone
        ]);
...
<a href="' . URL('/DeliveryChallanDelete/' . $row->InvoiceMasterID) . '"><i class="font-size-18 mdi mdi-trash-can-outline align-middle me-1 text-secondary"></i></a>
    ...
    public  function SalesInvoiceCreate()
    {
        ...
        // $tax = DB::table('tax')->get();
        $user = DB::table('users')->where('role','Agent')->get();
        $invoice_type = DB::table('invoice_type')->get();
        $branch = DB::table('branch')->get();
        $job = DB::table('job')->get();
    ...
    public  function InvoiceCreateAuto($id,$paymentno)
    {
        ...
        $vhno = DB::table('invoice_master')
         ...
         // get payment plan from estiamte with id and payment no from paymetno

         $payment_plan = DB::table('payment_plan')->where('EstimateMasterID',$id)->where('PaymentNo',$paymentno)->first();

 
        return view('invoice.sale_invoice_auto_create', compact('invoice_type', 'items', 'vhno', 'party', 'pagetitle', 'item', 'user', 'invoice_master', 'invoice_detail', 'tax','unit','job','branch','payment_plan'));
    }
...
//  <a href="' . URL('/ExpenseViewPDF/' . $row->ExpenseMasterID) . '"><i class="font-size-18 me-1 mdi mdi-file-pdf-outline align-middle me-1 text-secondary"></i></a>
    ...
    public function sendEmail2()
    {
        ...
        $toEmail = 'habib@eits.ae';

        // Define the subject
        $subject = 'Simple Email Subject';

        // Send the email
        Mail::raw('This is a simple text email sent from Laravel.', function ($message) use ($toEmail, $subject) {
            $message->to($toEmail)
                    ->subject($subject);
        });

    return "Simple text email sent successfully! ".$toEmail;
        // Send email
        // Mail::to('habib@eits.ae')->send(new OrderShipped());
    
        // return "Order shipped email sent successfully!";
    }
} // end of controller
